...
import { GLASS_POPOVER_FRAME_CLASS_NAME } from "./glassPopoverFrame";
...
/** Shared donor frame; single source of truth lives in glassPopoverFrame. */
const FRAME = GLASS_POPOVER_FRAME_CLASS_NAME;
...
export function Popover({
  ...
  useEffect(() => {
    // The measurement pass is hidden and cannot accept focus. Focus once the
    // frame is visible, without stealing it again when placement changes.
    if (autoFocus && positioned) {
      const target =
        surface.current?.querySelector<HTMLElement>("[data-popover-autofocus]") ?? surface.current;
      target?.focus();
    }
  ...
  const placed: CSSProperties = position
    ...
    : {
        ...
        top: 0,
        width,
        ...(constrainHeight ? { maxHeight: maxHeight ?? "calc(100vh - 16px)" } : {}),
        visibility: "hidden",
      };
  ...
  return createPortal(
    <div
      ...
      className={bare ? "monocode-surface" : FRAME}
    >
