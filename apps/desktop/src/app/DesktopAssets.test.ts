  ...
  it.effect("uses canonical source-tree icons for unpackaged development", () =>
    Effect.gen(function* () {
      ...
      const fileSystemLayer = FileSystem.layerNoop({
        exists: (path) => Effect.succeed(String(path).includes("/assets/ghosty/")),
      });
      ...
      assert.match(Option.getOrThrow(icons.ico), /assets\/ghosty\/workbench-windows\.ico$/);
      assert.match(Option.getOrThrow(icons.png), /assets\/ghosty\/workbench-1024\.png$/);
