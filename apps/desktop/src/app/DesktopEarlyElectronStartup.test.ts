...
describe("DesktopEarlyElectronStartup", () => {
  ...
  it("keeps implicit development state under ~/.ghosty-workbench/dev when T3CODE_HOME is unset", () => {
    const preference = resolveEarlyLinuxPasswordStorePreference({
      env: {
        VITE_DEV_SERVER_URL: "http://127.0.0.1:5173",
      },
      ...
      joinPath,
      readFileString: (path) => {
        assert.equal(path, "/home/user/.ghosty-workbench/dev/desktop-settings.json");
